...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['buttonEdit'])) {
    $userId = $_POST['buttonEdit'];

    $stmt = $mysqli->prepare("SELECT * FROM admins WHERE id = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        $_SESSION['error_detected'] = '<h3 id="temp-content" style="color: red; text-align: center;">Utilisateur non trouvé.</h3>';
        header("location: admins.php");
        exit();
    }
} else {
    $_SESSION['error_detected'] = '<h3 id="temp-content" style="color: red; text-align: center;">Vous devez cliquer sur le bouton Modifier d\'un Admin pour atteindre cette page.</h3>';
    header("location: admins.php");
    exit();
}
...
<head>
    ...
    <link rel="stylesheet" href="CSS/modifier-admin.css?v=<?php echo time(); ?>">
    ...
    <title>La Cissonière, modifier les administrateurs du site</title>
</head>
...
<body>
    ...
    <main>
        <section id="section_1">
            <form id="edit" method="POST" action="update_admin">
                ...
                <input type="password" name="password" minlength="8" value="<?php echo htmlspecialchars($row['password']); ?>"><br>
